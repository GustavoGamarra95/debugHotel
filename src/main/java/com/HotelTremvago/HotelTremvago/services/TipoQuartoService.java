...
public class TipoQuartoService {
    ...
    public Optional<TipoQuartoEntity> findById(Long id){
        try{
            return Optional.of(tipoQuartoRepository.findById(id).orElseThrow());
        }catch(Exception e){
            System.out.println("Nao foi possivel encontrar um tipo de quarto com este ID: " + e.getMessage());
            return Optional.of(new TipoQuartoEntity());
        }
    }
